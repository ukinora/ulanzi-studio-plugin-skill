---
name: ulanzi-studio
description: Build plugins for Ulanzi D200H / D200 / D200X stream controllers using the Ulanzi Studio SDK. Covers manifest.json schema, $UD SDK API, action class pattern, property inspector, CORS (incl. a Cloudflare relay workaround), flicker-free canvas rendering, marquee scrolling, realtime polling, credential/security hardening, localization, debugging, and Inno Setup distribution. TRIGGER when user asks to create or modify a Ulanzi Studio plugin/app, build a stream deck action, work in a `com.ulanzi.*.ulanziPlugin` directory, or install/distribute a Ulanzi plugin.
origin: https://github.com/ukinora/ulanzi-studio-plugin-skill
license: See LICENSE
---
...
This skill ships with reference templates in the repository:

- `templates/installer/setup.iss` - Inno Setup script for one-click Windows installers (see the
  Distribution section above).
- `templates/.github/workflows/build-installer.yml` - GitHub Actions workflow that compiles the
  installer on each Release.

When packaging a plugin, copy these into the plugin's `installer/` and `.github/workflows/`
folders and edit the `#define` block as documented.
